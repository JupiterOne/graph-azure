/**
 * The number of resources to request per resource API call (pagination `$top`).
 */
export function getPageLimit(name: string, defaultLimit: number): number {
  return getEnvValue(`AZURE_AD_${name.toUpperCase()}_PAGE_LIMIT`, defaultLimit);
}
...
export function getBatchPages(name: string, defaultLimit: number): number {
  return getEnvValue(
    `AZURE_AD_${name.toUpperCase()}_BATCH_PAGES`,
    defaultLimit,
  );
}
